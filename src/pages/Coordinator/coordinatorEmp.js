...
const CoordinatorEmpPage = (props) => {
  ...
  const handleSearch = () => {
    ...
    if (search !== "") {
      (async () => {
        try {
          const searchResultGet = await searchEmpBookingApi(
            search,
            data.bookingDetail.id
          );
          // console.log(searchResultGet.data);
          setSearchResult(searchResultGet.data);
        } catch (error) {
          console.log("Không thể lấy list");
        }
      })();
    }
  };
        ...
        <div className="row">
          ...
          >
            <div>
              ...
              {searchResult ? (
                searchResult.listEmp.length > 0 ? (
                  searchResult.listEmp.map((itemEmp, index) => (
                    <div
...
const mapStateToProps = (state) => ({
  ...
  dataSearch: state.searchEmpBooking.table,
});
